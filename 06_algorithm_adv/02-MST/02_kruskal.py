...
def mst_kruskal(vertices, edges):
    '''
        1. 가중치를 오름차순 정렬
        2. 그 순서대로 간선들을 선택
        3. 2에서 선택한 간선의 시작, 종료 노드가 같은 대표자가 아니어야 함
        4. 2와 3을 선택된 간선이 n-1개가 될 때까지 반복함
    '''
    ...
    for edge in edges:
        s, e, w = edge  # 시작노드, 종료노드, 가중치
        '''
            가중치는 처음 오름차순 과정에서 역할을 다함 
            s,e가 같은 집합에 속해있는지 판별, 만약 같은 집합에 속해있지 않은 노드들이라면 합침
        '''
