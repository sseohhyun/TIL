def find_subset(start, current_subset, current_sum):
    # 더이상 의미 없는 상황에 대해 가지치기 함
    if current_sum > target_sum :
        return

    # 현재 부분 집합의 합이 target_sum과 일치하면 result에 추가
    if current_sum  == target_sum:
        # 원본 그대로 넣으면 복제본이 들어가서 곤란 -> 새로운 리스트 만들어서 집어넣기
    ...
    for idx in range(start, len(nums)):
        num = nums[idx]
        # 현재 선택한 수를 집합에 넣고, 값도 추가해서 다음 작업 진행
